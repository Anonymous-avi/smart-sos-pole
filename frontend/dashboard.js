const dom = {
	...
	gps: document.getElementById("gpsValue"),
...
function updateSensorCards() {
	...
	dom.gps.textContent = `${state.gps.lat.toFixed(5)}, ${state.gps.lon.toFixed(5)}`;
	...
	if (dom.systemStatus) {
		if (state.flameDetected || state.touchSOS || state.emergencyActive) {
...
function applySensorPayload(sensorData) {
	...
	state.emergencyActive = Boolean(sensorData.sos) || state.touchSOS;

	if (typeof sensorData.gps === "string") {
		const [latText, lonText] = sensorData.gps.split(",").map((value) => value.trim());
		const parsedLat = Number(latText);
		const parsedLon = Number(lonText);
		if (!Number.isNaN(parsedLat) && !Number.isNaN(parsedLon)) {
			state.gps.lat = parsedLat;
			state.gps.lon = parsedLon;
		}
	}

	if (sensorData.gps && typeof sensorData.gps === "object") {
		state.gps.lat = Number(sensorData.gps.lat ?? state.gps.lat);
		state.gps.lon = Number(sensorData.gps.lon ?? sensorData.gps.lng ?? state.gps.lon);
	}
...
async function fetchSensorData() {
	try {
		...
		if (wasConnected) {
			pushEvent("Sensor API connection lost.");
		}
		wasConnected = false;

		if (dom.systemStatus) {
			dom.systemStatus.textContent = "Backend Offline";
			dom.systemStatus.style.background = "rgba(255, 95, 95, 0.16)";
			dom.systemStatus.style.borderColor = "rgba(255, 95, 95, 0.46)";
			dom.systemStatus.style.color = "#ffd2d2";
		}
...
async function triggerEmergencySOS() {
	...
	dom.emergencyBtn.textContent = "Triggering...";
	...
	try {
		...
		pushEvent(result.message || "Manual emergency triggered from dashboard.");
		dom.emergencyBtn.textContent = "Emergency Active";
		dom.emergencyBtn.classList.add("active");
		dom.emergencyNote.textContent = "Emergency mode active. Authorities have been notified.";
		await fetchSensorData();
	} catch (_error) {
		dom.emergencyBtn.disabled = false;
		dom.emergencyBtn.textContent = "Trigger Emergency SOS";
		dom.emergencyBtn.classList.remove("active");
		dom.emergencyNote.textContent = "Failed to trigger emergency. Check backend connection.";
		pushEvent("Failed to trigger emergency SOS.");
	}
}
	...
	environmentChart = new Chart(envCtx, {
		...
		options: {
			...
			scales: {
				x: {
					ticks: { color: "#9db8df" },
					grid: { color: "rgba(157, 184, 223, 0.15)" },
				},
				y: {
					ticks: { color: "#9db8df" },
					grid: { color: "rgba(157, 184, 223, 0.15)" },
	...
	safetyChart = new Chart(safetyCtx, {
		...
		options: {
			...
			scales: {
				x: {
					ticks: { color: "#9db8df" },
					grid: { color: "rgba(157, 184, 223, 0.12)" },
				},
				y: {
					min: 0,
					max: 1,
